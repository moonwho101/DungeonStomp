...
class RadixSort {
  public:
	...
	__forceinline udword *GetIndices() const {
		return mIndices;
	}

	//! mIndices2 gets trashed on calling the sort routine, but otherwise you can recycle it the way you want.
	__forceinline udword *GetRecyclable() const {
		return mIndices2;
	}
	...
	__forceinline udword GetNbTotalCalls() const {
		return mTotalCalls;
	}
	//! Returns the number of premature exits due to temporal coherence.
	__forceinline udword GetNbHits() const {
		return mNbHits;
	}
