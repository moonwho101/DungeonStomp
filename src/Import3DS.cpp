...
BOOL C3DS::Import3DS(HWND hwnd, char* filename, int pmodel_id, float scale)
{
	...
	*/
	...
	for (frame_num = 0; frame_num < total_num_frames; frame_num++)
	{
		...
		for (i = 0; i < total_num_verts; i++)
		{
			pCMyApp->pmdata[pmodel_id].w[frame_num][i].x = fverts[i][1];
			pCMyApp->pmdata[pmodel_id].w[frame_num][i].y = fverts[i][0];
...
void C3DS::AddMapName(HWND hwnd, FILE* fp, int pmodel_id)
{
	...
	{
		if (_strcmpi(map_name, pCMyApp->TexMap[i].tex_alias_name) == 0)
		{
