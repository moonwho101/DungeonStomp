...
HRESULT CD3DApplication::Create(HINSTANCE hInst, TCHAR *strCmdLine) {
	...
	m_hWnd = CreateWindow(_T("D3D Window"), m_strWindowTitle,
	                      ...
	                      CW_USEDEFAULT, CW_USEDEFAULT, 800, 600, 0L,
	                      LoadMenu(hInst, MAKEINTRESOURCE(IDR_MENU)),
	                      hInst, 0L);

	gmenuhandle = LoadMenu(hInst, MAKEINTRESOURCE(IDR_MENU1));
	gmenuhandle2 = LoadMenu(hInst, MAKEINTRESOURCE(IDR_MENU));
...
LRESULT CD3DApplication::MsgProc(HWND hWnd, UINT uMsg, WPARAM wParam,
	...
	case WM_COMMAND:
		...
		case IDM_SAVESPOT:
			pCMyApp->SaveFileSpot();

			return 0;

		case IDM_LOADSPOT:
			pCMyApp->LoadFileSpot();
			return 0;
		...
		case IDM_RELOADMAP:
			...
			if (a > 0) {
				...
				sprintf_s(pCMyApp->gActionMessage, "Loading %s", cname);
...
VOID CD3DApplication::ShowStats() {
	...
	//    OutputText((DWORD)80.0f, (DWORD) 80.0f, buffer );
}
