...
class matrix {
  public:
	...
	inline const matrix<N, P, T> operator>>(const matrix<M, P, T> &m) const {
		return concat(m);
	}
	template <unsigned int P>
	inline operator>>=(const matrix<M, P, T> &m) {
		*this = concat(m);
	}
	template <unsigned int P>
	...
	inline const matrix<N, 1, T> operator>>(const matrix<M, 1, T> &m) const {
		return concat(m);
	}
	inline int operator>>=(const matrix<M, 1, T> &m) {
		*this = int concat(m);
	}
	...
	inline const matrix<N, 2, T> operator>>(const matrix<M, 2, T> &m) const {
		return concat(m);
	}
	inline int operator>>=(const matrix<M, 2, T> &m) {
		*this = concat(m);
	}
	...
	inline const matrix<N, 3, T> operator>>(const matrix<M, 3, T> &m) const {
		return concat(m);
	}
	inline int operator>>=(const matrix<M, 3, T> &m) {
		*this = concat(m);
	}
	...
	inline const matrix<N, 4, T> operator>>(const matrix<M, 4, T> &m) const {
		return concat(m);
	}
	inline int operator>>=(const matrix<M, 4, T> &m) {
		*this = concat(m);
	}
	...
	inline T determinant() {
		...
		matrix &m = *this;
		return (m(0, 0) * m(1, 1) - m(1, 0) * m(0, 1)) * (m(2, 2) * m(3, 3) - m(3, 2) * m(2, 3)) - (m(0, 0) * m(2, 1) - m(2, 0) * m(0, 1)) * (m(1, 2) * m(3, 3) - m(3, 2) * m(1, 3)) + (m(0, 0) * m(3, 1) - m(3, 0) * m(0, 1)) * (m(1, 2) * m(2, 3) - m(2, 2) * m(1, 3)) + (m(1, 0) * m(2, 1) - m(2, 0) * m(1, 1)) * (m(0, 2) * m(3, 3) - m(3, 2) * m(0, 3)) - (m(1, 0) * m(3, 1) - m(3, 0) * m(1, 1)) * (m(0, 2) * m(2, 3) - m(2, 2) * m(0, 3)) + (m(2, 0) * m(3, 1) - m(3, 0) * m(2, 1)) * (m(0, 2) * m(1, 3) - m(1, 2) * m(0, 3));
	}
	...
	inline void fill(const T &value) {
		T *ptr = _data;
		for (unsigned int i = 0; i < N * M; i++, ptr++)
			*ptr = value;
	}
...
inline const matrix<N, M, T> operator*(const T &value, const matrix<N, M, T> &m) {
	return m * value;
}

template <unsigned int N, unsigned int M, class T>
inline const matrix<N, M, T> operator/(const T &value, const matrix<N, M, T> &m) {
	return m.inverseDivide(value);
}
...
template <unsigned int N, unsigned int M, class T>
inline const matrix<N, M, T> operator-(const T &value, const matrix<N, M, T> &m) {
	return m.inverseSubtract(value);
}
