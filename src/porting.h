...
struct CDC {
	...
	HFONT SelectFont(HFONT newfont) {
		return (HFONT)SelectObject(hdc, newfont);
	}
	HBITMAP SelectBitmap(HBITMAP newbitmap) {
		return (HBITMAP)SelectObject(hdc, newbitmap);
	}
	HDC hdc;
};
